...
const getdata = async (val: string) => {
    obj.value = val;
    const { data } = await QueryOfficialWebsiteCustomerOrderCount({
        officialWebsiteOrders: val,
        pageIndex: 1,
        pageSize: 30,
    });
    ...
    for (let key in data.data) {
        tablist.value[0].num =
            Number(tablist.value[0].num) + Number(data.data[key]);
    }
};
